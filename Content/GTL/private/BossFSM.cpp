...
void UBossFSM::IdleState()
{
	...
	if (currentTime > idleDelayTime && BossAnim != nullptr) // 현재 1초
	{
		...
		case 2: break;
		case 3: mBossState = EEnemyState::Blink; isIdle = false; BossAnim->BossAnimState = mBossState; currentTime = 0; break;
...
void UBossFSM::DeadState()
{
	...
	currentTime += GetWorld()->DeltaTimeSeconds;
	if (currentTime > DieDelayTime && isDead)
	{
...
void UBossFSM::DamageState()
{
	...
	else if (hitcnt >= 2 && BossAnim->BossAnimState != EEnemyState::Dead) // 두번 이상 맞으면 바로 순간이동
	{
		//GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Green, FString::Printf(TEXT("3")));
		BossAnim->BossAnimState = EEnemyState::Blink;
